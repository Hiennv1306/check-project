...
class IndexController extends Controller {
	...
	*/
	...
	public function getProduct(Request $req)
	{
		...
		return view('templates.productlist_tpl', compact('title','keyword','description','products', 'com','cate_pro','tintucs','selected','productSale'));
	}
	...
	public function getProductChild(Request $alias){
	...
	public function getProductMax($alias){

		$product_list = DB::table('product_categories')->select()->where('status',1)->where('alias',$alias)->first();
		if(isset($product_list)){
			$cate_parent = DB::table('product_categories')->where('status',1)->where('id',$product_list->parent_id)->first();
			$cateChilds = DB::table('product_categories')->where('parent_id',$product_list->id)->get();

			$array_cate[] = $product_list->id;
			if($cateChilds){
				foreach ($cateChilds  as  $cate) {
					$array_cate[] = $cate->id;
				}
			}
			$products = DB::table('products')->where('status',1)->whereIn('cate_id', $array_cate)->paginate(16);

	
		}
	
		return view('templates.productlist_tpl', compact('product_list','products'));
	}
	...
	public function getProductDetail($id, Request $req)
	{
		...
		if(!empty($product_detail)){
			$banner_danhmuc = DB::table('lienket')->select()->where('status',1)->where('com','')->where('link','san-pham')->get()->first();
			// sản phẩm đã xem
	...
	public function search(Request $request)
	{
		...
		$cate_id = $request->product_cat;
		$query = DB::table('products')
			->where('name', 'LIKE', '%' . $txtSearch . '%')
			->where('status',1);			

		if($cate_id){
			$cateChilds = DB::table('product_categories')->where('parent_id', $cate_id)->get();
			$array_cate[] = $cate_id;
        	if($cateChilds){
        		foreach($cateChilds as $cate){
        			$array_cate[] = $cate->id;
        	...
        	$query = $query->whereIn('cate_id',$array_cate);
		}
		$data = $query->orderBy('id','DESC')->get();
		$title = "Tìm kiếm: ".$txtSearch;
		$keyword = "Tìm kiếm: ".$txtSearch;
		$description = "Tìm kiếm: ".$txtSearch;
		
		return view('templates.search_tpl', compact('keyword','description','title','com','data','cate_id','txtSearch'));
	}
	...
	public function getNewsDetail($id)
	{
		...
		if(!empty($news_detail)){			
			$cate_pro = DB::table('product_categories')->where('status',1)->where('parent_id',0)->orderby('id','asc')->get();
			$baiviet_khac = DB::table('news')->where('status',1)->where('id','<>', $news_detail->id)->where('com','dich-vu')->orderby('id','desc')->take(5)->get();
	...
	public function getCart()
	{
		...
		$img_share = '';
		// End cấu hình SEO
		return view('templates.giohang_tpl', compact('doitac','product_cart','district','product_noibat','province','keyword','description','count','title','img_share','total', 'bank'));
	}

	public function addCart(Request $req)
	{
		$data = $req->only('product_id','cart');
